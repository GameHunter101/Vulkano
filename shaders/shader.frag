...
float geometry(vec3 light,vec3 normal,vec3 view,float roughness){
    ...
    return.5/max(.01,mix(2*NdotL*NdotV,NdotL+NdotV,roughness));
}
...
vec3 compute_radiance(vec3 irradiance,vec3 light_direction,vec3 normal,vec3 camera_direction,vec3 surface_color){
    ...
    vec3 F0=mix(vec3(.03),surface_color,vec3(metallic));
    ...
    vec3 halfvector=normalize(.5*(camera_direction+light_direction));
...
void main(){
	...
	for (int i=0;i<number_directional;i++){
		...
		L += compute_radiance(dlight.irradiance, dlight.direction_to_light, normal, direction_to_camera, color_in);
	}
	...
	for (int i=0;i<number_point;i++){	
		...
		L += compute_radiance(irradiance, direction_to_light, normal, direction_to_camera, color_in);
	}
